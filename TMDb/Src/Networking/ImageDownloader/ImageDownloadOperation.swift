...
class ImageDownloadOperation: Operation {
  ...
  typealias CompletionHandlerClosure = (_ successFullySaved: Bool, ImageInfo) -> Void
  private var completionHandler: CompletionHandlerClosure
  
  
  init(imageInfo: ImageInfo, session: URLSession, completionHandler: @escaping CompletionHandlerClosure) {
  ...
  override func start() {
    ...
    self.downloadTask = self.session.downloadTask(with: self.imageInfo.serverUrl, completionHandler: {[weak self] (url, response, error) in
      ...
      if error == nil {
        if let res = response as? HTTPURLResponse {
          if res.statusCode == 200 {
            ...
            if let srcUrl = url, let destUrl = self?.imageInfo.diskUrl {
              do {
      ...
      if let completionHandler = self?.completionHandler {
        completionHandler(success, self!.imageInfo) // we have self here so unwraping it
      }
    })
